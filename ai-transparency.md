...
Proxmox snapshot path to find the one primitive to wrap (`foreach_volume_full`); typing the
module, the wrapper, the scripts, these docs and the unit tests to the human's design;
laying out options at each fork for the human to choose from; and grinding through the variants
and the off-node checks. On the human's own test node, with their
explicit go-ahead at each step, it also carried out the install and ran the full test matrix
(create, rollback verified at the file level, delete, the BINDSNAP-FORCE-RUNNING and BINDSNAP-UNSUPPORTED paths) and read
the upstream checksums to seed the table. None of the decisions, and none of the accountability,
were the AI's -- the human directed and approved every action and owns the result.
...
engages for (bind/device mounts versus plain volumes), and the status messages.
...
refusal, the `BINDSNAP-EXCLUDE` cases, and the task-log summaries. Beyond that test node, it
runs in the author's own production cluster on the same pve-container build.

## Checking it yourself

The module is a single, heavily commented file. The only part that does anything clever is the
thin filter around `foreach_volume_full`; everything else hands off to the live Proxmox methods.
...
`PVE::LXC::Config`, and puts a thin wrapper in its place; `uninstall.sh` takes it all back out.
